...
class commonQA(TextGenPool):
    @classmethod
    def prepare(cls, split: str, suffix: str = "",
                prefix: str = "",):
        dataset = load_dataset('commonsense_qa',name='commonsense_qa')
        # print("str, str", str)
        ...
        for ix, item in tqdm(enumerate(dataset[dataset_split]),
            ...
            # print(item["choices"])
            # options = {}
            # for opt in range(len(item["choices"]['label'])):
            #     options[opt["label"]] = opt["text"]
            # print({option["label"]: option["text"] for option in item["choices"]})
            # print(str({option["label"]: option["text"] for option in item['choices']}))
            # print("item", item)
            if item["answerKey"] == '':
                answer = 'there is no answer'
            # print("index", item["choices"]["label"].index(item["answerKey"]))
            else:
                answer = item["choices"]["text"][item["choices"]["label"].index(item["answerKey"])]
                
            sample = Sample(id=f"{split}_{ix}",
                                prompt_or_input_text=prefix +
                                item["question"] + suffix + str(item["choices"]["text"]),
                                references= "the answer is " + answer
                                )
            samples.append(sample)
